...
int 	check_perm(char *path)
{
	...
	t_element *create_child_list(char *path, int parent) //On recoit juste le chemin a ouvrir
	{
		struct dirent *dent;
		struct stat fileinfo;
		t_element *child = new_list();
		DIR *dir;
		char *full_path;
	...
	void parent_to_childe(t_element *parent, char *path2, int i) //ajout du path pour la recursive
	{
		DIR *dir;
		t_element *child = new_list();
		int alloc = 0;
